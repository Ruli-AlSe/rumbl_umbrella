defmodule Rumbl.AccountsTest do
  ...
  describe "register_user/1" do
    ...
    test "with valid data inserts user" do
      ...
      assert [%User{id: îd}] = Accounts.list_users()
    end
  ...
  describe "authenticate_by_username_and_pass/2" do
    ...
    test "returns unauthrized error with invalid password", %{user: user} do
      assert {:error, :unauthorized} = Accounts.authenticate_by_username_and_pass(user.username, "wrong_password")
    end

    test "return not found error with no matching user for username", %{user: user} do
